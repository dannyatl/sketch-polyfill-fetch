...
// progressCallback: called repeatedly with upload progress, as a decimal (e.g. 0.5 for 50%)
function fetch (urlString, options, progressCallback) {
  options = options || {}
    ...
    if (!DelegateClass) {
      DelegateClass = ObjCClass({
        ...
        'connection:didReceiveResponse:': function (connection, httpResponse) {
          ...
          this.contentSize = httpResponse.expectedContentLength
        ...
        'connection:didReceiveData:': function (connection, data) {
          ...
          progressCallback && progressCallback(this.data.length / this.contentSize);
        }
      })
    }
